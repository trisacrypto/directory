...
func main() {
	...
	app.Flags = []cli.Flag{
		&cli.StringFlag{
			...
			EnvVars: []string{sectigo.UsernameEnv},
		},
		&cli.StringFlag{
			Name:    "password",
			Aliases: []string{"p"},
			Usage:   "API access login password",
			EnvVars: []string{sectigo.PasswordEnv},
		},
		&cli.StringFlag{
			Name:    "profile",
			Aliases: []string{"P"},
...
func createSingle(c *cli.Context) (err error) {
	...
	if domain == "" {
		return cli.Exit("must specify domain name of cert subject", 1)
	}

	authority := c.Int("authority")
	if authority == 0 {
		return cli.Exit("must specify authority ID", 1)
	}
...
func uploadCSR(c *cli.Context) (err error) {
	if c.NArg() != 1 {
		return cli.Exit("specify the path to one CSR for upload", 1)
	}
	...
	if profileId == 0 {
		return cli.Exit("specify the profile ID to sign the cert under", 1)
	}

	// Load the CSR data from the file
	var csrData []byte
	if csrData, err = os.ReadFile(path); err != nil {
		return cli.Exit(fmt.Errorf("could not read %s: %s", path, err), 1)
	}
...
func batches(c *cli.Context) (err error) {
	...
	return cli.Exit("specify batch id to get information", 1)
}
...
func download(c *cli.Context) (err error) {
	...
	batch := c.Int("id")
	if batch == 0 {
		return cli.Exit("must specify batch id for download", 1)
	}
...
func profiles(c *cli.Context) (err error) {
	...
	if c.Bool("params") {
		return cli.Exit("must specify id to get profile params", 1)
	}
...
func revokeCert(c *cli.Context) (err error) {
	pid := c.Int("profile")
	if pid == 0 {
		return cli.Exit("must specify profile id", 1)
	}
	...
	if sn == "" {
		return cli.Exit("must specify serial number of certificate", 1)
	}
