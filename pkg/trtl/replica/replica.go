...
func (r *Service) Gossip(stream replica.Replication_GossipServer) (err error) {
	...
	logctx := sentry.With(stream.Context()).
		Str("service", "gossip").
		Bool("remote", true)
...
func (r *Service) remotePhase1(ctx context.Context, wg *sync.WaitGroup, logctx *sentry.Logger, stream replica.Replication_GossipServer, sender *streamSender) {
	defer wg.Done()
	logctx.Trace().Msg("starting phase 1")
...
gossip:
	for {
		...
		case replica.Sync_CHECK:
			...
			if phase3 {
				logctx.Warn().Msg("received check message in phase 3 from client")
				continue gossip
			}
		...
		case replica.Sync_COMPLETE:
			...
			once.Do(func() {
				wg.Add(1)
				go r.remotePhase2(ctx, wg, logctx, &seen, sender, &updates)
			})
...
func (r *Service) remotePhase2(ctx context.Context, wg *sync.WaitGroup, logctx *sentry.Logger, seen *nsmap, sender *streamSender, updates *uint64) {
	// Start a timer to track latency
	start := time.Now()

	// Ensure that this routine signals when it exists
	defer wg.Done()
	logctx.Trace().Msg("starting remote phase 2")
