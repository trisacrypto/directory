...
func TestDoubleCookies(t *testing.T) {
	...
	router.GET("/protect", func(c *gin.Context) {
		err := auth.SetDoubleCookieToken(c, "", time.Now().Add(10*time.Minute))
		require.NoError(t, err)
		c.JSON(http.StatusOK, gin.H{"success": true})
	})

	// Add a route that requires double cookie submit
	router.POST("/action", auth.DoubleCookie(), func(c *gin.Context) {
	...
	require.Equal(t, auth.ErrCSRFVerification.Error(), data["error"].(string))
	...
	reqb.Header.Set(auth.CSRFHeader, "")
	require.NoError(t, err)
	...
	reqc.Header.Set(auth.CSRFHeader, "not a valid csrf token")
	require.NoError(t, err)
	...
	for i, req := range testCases {
		...
		require.Equal(t, auth.ErrCSRFVerification.Error(), data["error"].(string), "bad request %d failed", i)
	...
	req, err = http.NewRequest(http.MethodPost, srv.URL+"/action", nil)
	req.Header.Set(auth.CSRFHeader, cookieToken)
	require.NoError(t, err)
