...
import { Status as VerificationStatus, StatusLabel } from '@/constants/index';
import { actionType, useModal } from '@/contexts/modal';

dayjs.extend(relativeTime);

export const getRecentVasps = (vasps) =>
  vasps?.filter((vasp) => vasp.verification_status === VerificationStatus.PENDING_REVIEW);
            ...
            <tr key={vasp.id}>
              ...
              <td>
                ...
                <span className="badge badge-warning-lighten">
                  {StatusLabel[vasp?.verification_status]}
                </span>
              </td>
              <td>
  ...
  return (
    <Card className="w-100">
        ...
        {(isLoading || !recentVasps) && <OvalLoader />}
              {recentVasps?.length ? (
          <PendingReviewsTable data={recentVasps} />
        ) : (
