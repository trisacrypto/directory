...
import {
  ...
  updateBusinessInfosResponseError,
  updateBusinessInfosResponseSuccess,
  UpdateIvms101ActionTypes,
  updateIvms101ResponseError,
  updateIvms101ResponseSuccess,
  UpdateTrisaImplementationDetailsActionTypes,
  updateTrisaImplementationDetailsResponseError,
  updateTrisaImplementationDetailsResponseSuccess,
...
function* fetchVaspDetails({ payload: { id, history } }) {
  ...
  try {
    ...
    // toast.error(`VASP with ID ${id} doesn't exist`, { id })
    history.push('/error-404', { error: `VASP with ID ${id} doesn't exist` });
    NProgress.done();
  }
}
...
export function* updateTrisaDetailsSaga() {
  yield takeEvery(
    UpdateTrisaImplementationDetailsActionTypes.UPDATE_TRISA_DETAILS,
    updateTrisaDetails
  );
}
