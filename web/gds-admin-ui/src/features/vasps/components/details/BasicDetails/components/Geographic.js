...
export const renderLines = (address) => (
  <address data-testid="addressLine">
    {address.address_line.map(
      (addressLine, index) => addressLine && <div key={index}>{addressLine} </div>
    )}
    ...
    <Col lg={6}>
      {data &&
        data.map((address, index) => (
          <Row key={index}>
            <p className="mb-1 fw-bold" data-testid="addressType">
              {AddressTypeHeaders[address?.address_type]} Address:
            </p>
