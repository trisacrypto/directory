...
import { ModalCloseButton, useModalContext } from '@/components/Modal';
...
function ReviewForm() {
    ...
    return !verificationToken ? (
        <OvalLoader title="retrieving verification token..." />
    ) : (
        <Form onSubmit={handleSubmit(onSubmit)}>
                ...
                {isReject && (
                    <Form.Group className="mt-2">
                        ...
                        {errors.reject_reason ? (
                            <Form.Control.Feedback type="invalid">{errors.reject_reason.message}</Form.Control.Feedback>
                        ) : (
                            <Form.Text type="invalid">* Please supply the rejection reason</Form.Text>
                        )}
                    </Form.Group>
                )}
            </Row>
            <Row>
                ...
                <Col>
                    <Button type="submit" className="mt-3 w-100" disabled={isLoading || !isDirty}>
                        Submit
                    </Button>
                </Col>
            </Row>
