  ...
  national_identification: yup.object().shape({
    registration_authority: yup
      ...
      .test(
        ...
        function (value) {
          if (
            this.parent.national_identifier_type !== 'NATIONAL_IDENTIFIER_TYPE_CODE_LEIX' &&
            value === ''
          ) {
            return false;
          }
...
function Ivms101RecordForm({ data }) {
  ...
  const isRegistrationAuthorityDisable = React.useCallback(
    () => nationalIdentifierType === 'NATIONAL_IDENTIFIER_TYPE_CODE_LEIX',
    [nationalIdentifierType]
  );
  const _typeahead = React.useRef();
  React.useEffect(() => {
  ...
  return (
    <div>
      ...
      <p>
        ...
        as KYC information exchanged in TRISA transfers.
      </p>
      ...
      <Form onSubmit={handleSubmit(onSubmit)}>
        ...
        <FormGroup>
          ...
          <Form.Text>
            TRISA specific identity number (UUID), only supplied if you're updating an existing
          ...
          <Controller
            ...
            render={({ field }) => (
              <Typeahead
                ...
                isValid={
                  !isRegistrationAuthorityDisable() &&
