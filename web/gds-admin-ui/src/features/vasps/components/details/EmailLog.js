        ...
        <Card>
            <Card.Body>
                ...
                {Array.isArray(data) && data.length ? (
                            ...
                            {sortByTimestamp(data).map((log, idx) => (
                                <TimelineItem key={idx}>
                                    ...
                                    <div className="timeline-item-info mb-3">
                                        <h5 className="text-primary mb-1" data-testid="email-log-reason" href="/">
                                            {log?.reason || 'N/A'}
                                        </h5>
                                        <small className="d-block" data-testid="email-log-contact-line">
                                            Recipient:{' '}
                                            <span className="fst-italic" data-testid="email-log-contact">
                                                {log?.recipient || 'No recipient'}
                                            </span>
                                        </small>
                                        <small data-testid="email-log-subject-line">
                                            Subject:{' '}
                                            <span className="fst-italic" data-testid="email-log-subject">
                                                {log?.subject || 'No subject'}
                                            </span>
                                        </small>
                                        <small className="d-block" data-testid="email-log-timestamp">
                                            Date: {dayjs(log?.timestamp).toLocaleString()}
                                        </small>
                                    </div>
                                </TimelineItem>
                            ))}
                    ...
                    <p className="fst-italic fs-6" data-testid="no-data">
                        No Email Log
                    </p>
                )}
            </Card.Body>
