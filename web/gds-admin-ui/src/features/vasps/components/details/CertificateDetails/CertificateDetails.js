...
function CertificateDetails({ data }) {
    ...
    return (
        <Card>
            {!data ? (
                <Card.Body className="fst-italic text-muted">Identity certificate not available</Card.Body>
            ) : (
                <Card.Body>
                    ...
                    <span data-testid="certificate-state" className={`badge rounded-pill px-1 ${getBadgeClassName()}`}>
                        {getBadgeLabel()}
                    </span>

                    <p className="fw-bold mb-1 mt-3">
                        Serial number: <span className="fw-normal">{formatDisplayedData(data?.serial_number)}</span>
                    </p>
                    <p
                        data-testid="certificate-expiration-date"
                        className={`mb-1 ${getExpireColorStyle(data?.not_after)}`}>
                        <span className="fw-bold">Expires:</span> {new Date(data?.not_after).toUTCString()}
                    </p>
                    <p className="mb-1">
                        <span className="fw-bold">Issuer: </span>
                        {data?.issuer?.common_name}
                    </p>
                    <p className="mb-1">
                        <span className="fw-bold">Subject: </span>
                        {data?.subject?.common_name}
                    </p>
                    ...
                    <Row>
                        <Col sm={12}>
                            <FileInformationCard
                                file={data?.data}
                                name="Public Identity Key"
                                ext=".PEM"
                                onDownload={() => handlePublicIdentityKeyDownloadClick(data?.data)}
                            />
                            <FileInformationCard
                                file={data?.chain}
                                name="TRISA Trust Chain (CA)"
                                ext=".GZ"
                                onDownload={() => handleTrustChainDownloadClick(data?.chain)}
                            />
                        </Col>
                        <Col>
                            <Details data={data} />
